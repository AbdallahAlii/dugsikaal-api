...
from app.auth.service.auth_service import AuthService
from app.common.models.base import StatusEnum
from app.security.rbac_guards import attach_auth_context
...
def before_request_session_auth():
    ...
    user_id = session.get("user_id")
    if not user_id:
        return  # require_login_globally handles protected endpoints

    try:
        redis_ok = redis_kv.ping()

        if redis_ok:
            # HARD GATE 1: cached account status (only when Redis is healthy)
            cached_status = get_cached_user_status(int(user_id))
            if cached_status and cached_status != StatusEnum.ACTIVE.value:
                session.clear()
                abort(403, description="Your account is locked.")

            # HARD GATE 2: session indexing (only when Redis is healthy)
            if not is_session_indexed(int(user_id)):
                session.clear()
                abort(401, description="Invalid session. Please log in again.")
        else:
            # Redis down: do not lock users out; fall back to DB validation through profile load
            log.warning("Redis unavailable: skipping Redis-only session gates")

        prof = _auth.get_cached_profile(int(user_id))  # cache layer is safe now
        ...
        if prof and prof.get("ok"):
            ...
            try:
                ...
                log.warning("attach_auth_context failed for user %s: %s", user_id, e, exc_info=True)
            return
        ...
        log.error("Auth middleware exception for user %s: %s", user_id, e, exc_info=True)
        session.clear()
        abort(500)