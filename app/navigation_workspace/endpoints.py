...
from app.navigation_workspace.schemas import (
    ...
    CompanyWorkspaceVisibilityIn, CompanyPackageToggleIn,
)
...
# -------------------------------------------------------------
@bp.post("/admin/companies/<int:company_id>/packages/<string:package_slug>")
def upsert_company_package(company_id: int, package_slug: str):
    """
    System Admin: enable/disable a single package for a company.

    Body: CompanyPackageToggleIn
    {
      "is_enabled": true/false,
      "valid_from": "2025-11-27T00:00:00",
      "valid_until": null
    }
    """
    try:
        ...
        body = CompanyPackageToggleIn(**body_json)

        svc = PackageService()
        out = svc.upsert_company_package_for_company(
        ...
        db.session.commit()
        return api_success(out.model_dump(), message="Company package updated.")
    except PermissionError:
        return api_error("Unauthorized", status_code=401)
        ...
        db.session.rollback()
        return api_error(f"Failed to update company package: {e}", status_code=500)

@bp.post("/admin/companies/<int:company_id>/packages")
def set_company_packages(company_id: int):
    """
    Bulk ERP-style update of packages for a company.
    ...
    try:
        ...
        svc = PackageService()
        out = svc.bulk_set_company_packages_for_company(
            company_id=company_id,
            body=body,
        )
