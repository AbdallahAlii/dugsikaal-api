...
#
# # ------- CHANGE: datetime + SQLA type helpers for date-range filtering -------
...
#
# from ...common.date_utils import parse_date_flex, ACCEPTED_FORMATS_HUMAN
#
...
#         return False
#
# # CHANGE: resolve which date column to use
...
#     return None, None
#
# # CHANGE: collect date params (exact or range), with aliases
# def _extract_date_params(filters: Dict[str, Any], field_name: Optional[str]) -> Tuple[Optional[str], Optional[str], Optional[str]]:
...
# # CHANGE: strip date keys so apply_filters_sa doesn't add a conflicting "=" filter
...
#
# # CHANGE: apply date filters first (generic)
...
# #
# #     # IMPORTANT: prevent apply_filters_sa from re-applying these as "="
...
#
#             # 2) date filters (generic) — CHANGE: uses a local copy
#             _filters_local = dict(filters or {})
#             q = _apply_generic_date_filters_if_any(q, cfg, _filters_local)
#
#             # 3) remaining filters (simple ops) — unchanged
...
#
# # -------------------- DETAIL repository (unchanged) --------------------
# class DetailRepository:
#     def _resolve_record_id(
#         self, session: Session, cfg: DetailConfig, ctx: AffiliationContext, by: str, ident: str
#     ) -> Optional[int]:
#         # Named lookups via resolver_map
#         if cfg.resolver_map:
#             # Use explicit ?by= if provided; otherwise config default_by; otherwise fallback.
