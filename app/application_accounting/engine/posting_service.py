...
class PostingService:
    ...
    def post(self, ctx: PostingContext) -> JournalEntry:
        ...
        with lock_doc(self.s, ctx.company_id, ctx.source_doctype_id, ctx.source_doc_id):
            ...
            # --- helper: convert (effect, signed amount) -> (debit, credit) ---
            def _split_amount(effect: str, raw_amount: Decimal):
                amt = Decimal(str(raw_amount or 0))
                if amt == 0:
                    return Decimal("0"), Decimal("0")

                # Positive amount -> normal direction
                if amt > 0:
                ...
                # Negative amount -> reverse direction (used for returns / credit notes)
                mag = abs(amt)
                if effect.upper().startswith("D"):
                    # template says "DEBIT", but negative => actually CREDIT
                    return Decimal("0"), mag
                else:
                    # template says "CREDIT", but negative => actually DEBIT
                    return mag, Decimal("0")

            def _append_line(account_id: int, raw_amount: Decimal, effect: str, party_ok: bool):
                nonlocal total_dr, total_cr, line_objs
                debit, credit = _split_amount(effect, raw_amount)
