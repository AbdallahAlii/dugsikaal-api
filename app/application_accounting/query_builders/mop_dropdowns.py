...
# ---- access helpers (used by policies; dropdowns are company-wide) ----
def _is_super_admin(ctx: AffiliationContext) -> bool:
    roles = getattr(ctx, "roles", []) or []
    return "Super Admin" in roles
...
def _has_company_wide_access(ctx: AffiliationContext) -> bool:
    # kept for future use by policies; dropdown visibility is company-wide anyway
    return bool(getattr(ctx, "is_system_admin", False) or _is_super_admin(ctx) or _is_company_owner(ctx))


def _br_ids(ctx: AffiliationContext) -> list[int]:
    # not used for dropdown visibility anymore (company-wide)
    return list(getattr(ctx, "branch_ids", []) or [])
...
def build_vat_account_dropdown(session: Session, ctx: AffiliationContext, params: Mapping[str, Any]):
    ...
    ensure_scope_by_ids(context=ctx, target_company_id=co_id, target_branch_id=None)
...
# ───────────────────── MoP → Accounts (dependent dropdown) ─────────────────────
def build_mop_accounts_dropdown(session: Session, ctx: AffiliationContext, params: Mapping[str, Any]):
    """
    Dependent dropdown: requires 'mop_id' (or 'mode_of_payment_id').
    ...
    if role_raw:
        try:
            role_val = AccountUseRoleEnum(role_raw)
        except Exception:
            role_val = None  # ignore bad role; behave as if role not provided
...
def _build_customers_or_suppliers_dropdown(
    ...
    """
    Company-wide Customers or Suppliers.
    Branch filter is ONLY applied if explicitly passed in params['branch_id'].
    """
    co_id = _co(ctx)
    if not co_id:
        return select(Party.id.label("value")).where(literal(False))
    ...
    # ONLY if caller explicitly filters by branch_id
    if explicit_branch_id is not None:
        try:
            q = q.where(Party.branch_id == int(explicit_branch_id))
        except Exception:
...
def _build_employees_dropdown(session: Session, ctx: AffiliationContext, params: Mapping[str, Any]):
    ...
    Branch filter is ONLY applied if explicitly passed in params['branch_id']
    (via EmployeeAssignment primary active branch).
    """
    co_id = _co(ctx)
    if not co_id:
        return select(Employee.id.label("value")).where(literal(False))
    ...
    # Employees: default ledger = Employee Advances (1151)
    ledger_id_sq = _ledger_id_sq_for_role(co_id, PartyRoleEnum.EMPLOYEE)
    ...
    # Branch filter only when explicitly requested
    if explicit_branch_id is not None:
        EA = EmployeeAssignment
        base = base.join(
            EA,
    ...
    return base.distinct(Employee.id).order_by(
        func.lower(Employee.full_name).asc(), Employee.code.asc()
    )


def _build_shareholders_dropdown(session: Session, ctx: AffiliationContext, params: Mapping[str, Any]):
    # If you later add a Shareholder table, mirror the pattern above and keep the ledger meta
    return select(literal(None).label("value")).where(literal(False))
...
def build_parties_dropdown(session: Session, ctx: AffiliationContext, params: Mapping[str, Any]):
    ...
    Company-wide by default; branch filter only if explicitly passed.

    Params:
      - party_type / party_kind / role: "Customer" | "Supplier" | "Employee" | "Shareholder"
...
# ───────────────────── Fiscal Years (for PCV etc.) ─────────────────────
def build_fiscal_years_dropdown(session: Session, ctx: AffiliationContext, params: Mapping[str, Any]):
    """
    List Fiscal Years for the current company (ERP-style).

    Optional params:
      - status: filter by FiscalYear.status (e.g. "OPEN", "CLOSED")
      - q: search by name
    """
    co_id = _co(ctx)
    if not co_id:
        return select(FiscalYear.id.label("value")).where(literal(False))

    ensure_scope_by_ids(context=ctx, target_company_id=co_id, target_branch_id=None)

    q = (
        select(
            FiscalYear.id.label("value"),
            FiscalYear.name.label("label"),
            FiscalYear.status.label("status"),
            FiscalYear.start_date.label("start_date"),
            FiscalYear.end_date.label("end_date"),
        )
        .where(FiscalYear.company_id == co_id)
    )

    status = params.get("status")
    if status:
        q = q.where(FiscalYear.status == status)

    term = (params.get("q") or "").strip()
    if term:
        like = f"%{term}%"
        q = q.where(FiscalYear.name.ilike(like))

    return q.order_by(FiscalYear.start_date.asc(), FiscalYear.name.asc())


# ───────────────────── PCV Closing Account Head ─────────────────────
def build_pcv_closing_accounts_dropdown(session: Session, ctx: AffiliationContext, params: Mapping[str, Any]):
    """
    Closing Account Head dropdown for Period Closing Voucher.

    Defaults:
      - Company-wide (current company).
      - Only enabled, non-group accounts.
      - By default restricts to EQUITY accounts (like Retained Earnings).

    Optional params:
      - account_type: override default (e.g. "EQUITY", "LIABILITY")
    ...
    account_type_param = params.get("account_type")
    ...
    # Default: Equity accounts (typical PCV closing head)
    if account_type_param:
        q = q.where(Account.account_type == account_type_param)
    else:
        q = q.where(Account.account_type == AccountTypeEnum.EQUITY)
