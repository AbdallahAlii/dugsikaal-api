    ...
    def cancel(
        ...
        try:
            with self.s.begin_nested():
                with lock_doc(self.s, exp.company_id, doctype_id, exp.id):
                    exp_locked = self._get_expense(expense_id, context, for_update=True)
                    if exp_locked.doc_status != DocStatusEnum.SUBMITTED:
                        raise BizValidationError(
                            "Only submitted Expenses can be cancelled."
                        )

                    # 🔄 Reverse the original auto JE(s) for this Expense
                    ctx_cancel = PostingContext(
                        ...
                        is_auto_generated=True,
                        remarks=(
                                f"Cancel Expense {exp_locked.code}"
                    ...
                    self.repo.mark_cancelled(exp_locked.id)

            self.s.commit()

            # ----- CACHE -----
            self._bump_expense_caches(
            ...
            try:
                ...
                log.exception(
                    "[cache] failed to bump COA balance after expense cancel"
                )
