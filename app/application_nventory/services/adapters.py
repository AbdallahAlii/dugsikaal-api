...
#
#     The HTTP layer already enforced permission for /api/data-imports/* via @require_permission,
#     so this context is only used inside the background import job.
#     """
#
#     # These are injected in pipeline._inject_context()
...
#     else:
#         # Default to a role that is allowed to create inventory master data
#         roles = {"Operations Manager"}
#
#     # -------- Affiliations --------
#     # We don't have the full affiliation graph here, and recreating it from DB would be overkill.
#     # For the import job, we just say: "this context is tied to this exact company".
#     #
#     # AffiliationContext.affiliations is usually a list of richer objects, but ensure_scope_by_ids
#     # only actually cares about "is this user allowed to work in this company?".
#     # Instead of trying to fake the full object structure, we will rely on is_system_admin=True
#     # for this import-only context (see below) and keep this as an empty list to avoid type issues.
#     affiliations = []
#
#     # -------- Permissions --------
...
#     #
#     # 🔑 KEY CHANGE:
#     # Mark this context as system-level for RBAC scope checks.
#     # This allows ensure_scope_by_ids(...) to pass for the DI.company_id, since
#     # the HTTP endpoint already enforced "Data Import" permissions for the real user.
#     is_system_admin = True
#
#     # User type can just be "user" for import flows
