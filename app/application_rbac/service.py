...
class RbacService:
    ...
    #         # generic, toast-friendly
    #         raise NotFound("Some roles were not found.")
    #
    #     # make sure caller can act on the target user (simple application_org/branch gate)
    #     tgt_company, tgt_branch, tgt_aff_id = self.repo.get_user_primary_affiliation(target_user_id)
    #     ensure_scope_by_ids(context=context, target_company_id=tgt_company, target_branch_id=tgt_branch)
    #
    ...
    #     # - custom     -> (context.company_id, None)
    ...
    #                 user_affiliation_id=None,  # global anchor
    #             )
    #         if del_pre:
    ...
    #             )
    #
    #         # SET for custom (company anchor, branch ignored)
    #         if custom_ids:
    #             if context.company_id is None:
    #                 # should not happen for tenants, but keep a guard
    #                 raise Forbidden("Only predefined or your-company roles.")
    #
    ...
    #             # link to target's affiliation only if same company
    #             link_aff = tgt_aff_id if (tgt_company == context.company_id) else None
    #
    #             for rid in add_cust:
    #                 self.repo.assign_user_role(
    #                     user_id=target_user_id,
