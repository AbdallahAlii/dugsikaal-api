...
#         if job_id:
#             di.job_id = job_id
#             # 🔹 Only force IN_PROGRESS when using a real async worker.
#             # For inline jobs ("inline"), run_import has already set the final status.
#             if job_id != "inline":
...
#         if job_id:
#             di.job_id = job_id
#             # Same logic as /start: only mark IN_PROGRESS for real async jobs.
#             if job_id != "inline":
#                 set_status(di, ImportStatus.IN_PROGRESS)
#             db.session.commit()
...
from app.security.rbac_guards import require_permission
from app.security.rbac_effective import AffiliationContext
from app.auth.deps import get_current_user  # ✅ added
...
def _ctx() -> AffiliationContext:
    """
    Resolve the authenticated user's AffiliationContext.

    We mirror the pattern you already use in selling endpoints:
    - Force auth to be loaded via get_current_user()
    - Read g.auth (set by your auth middleware)
    """
    _ = get_current_user()
