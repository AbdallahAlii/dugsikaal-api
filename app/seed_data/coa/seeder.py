...
#         return obj, True
#     except IntegrityError:
#         # Another transaction may have created it first; reload and continue.
#         db.rollback()
#         return db.scalar(select(model).filter_by(**filters)), False
#
#
...
#     root_name: str = "Root Chart of Accounts",
#     use_company_prefix_for_root: bool = True,
#     set_status_submitted: bool = True,   # kept for signature compatibility (ignored)
#     create_balances_for_leaves: bool = True,
# ) -> None:
#     """
#     Idempotent COA seed for a given company:
...
def _get_or_create(db: Session, model, *, defaults: Optional[dict] = None, **filters):
    """
    Helper used while seeding accounts / balances.

    IMPORTANT:
      - We no longer call db.rollback() here.
      - Any IntegrityError will be raised so that the outer service
        can rollback the whole transaction and return an error.

    This avoids partial rollbacks that silently removed the Company
    ...
    try:
        ...
        logger.exception(
            "IntegrityError in COA _get_or_create for %s with filters=%s",
...
def seed_chart_of_accounts(
    ...
    *,
    ...
    if not company:
        logger.error("Company id=%s not found; skipping COA seeding.", company_id)
        raise ValueError(f"Company id={company_id} not found while seeding COA")
