...
# app/seed_data/gl_templates/seeder.py
...
# Models (adjust import paths if your layout differs)
...
# DocumentType model (keep path as in your project)
from ...application_stock.stock_models import DocumentType
...
def _ensure_single_primary(
    ...
    *,
    company_id: int,
    doctype_id: int,
    keep_template_id: Optional[int] = None,
) -> None:
    """
    Demote all primaries for (company, doctype) except optionally one.
    Must be executed BEFORE inserting/setting a new primary to avoid
    the partial-unique constraint hit.
    """
    q = (
    ...
    if keep_template_id is not None:
        q = q.where(GLEntryTemplate.id != keep_template_id)
    db.execute(q)
    # Make sure the UPDATE is flushed before any INSERT that sets is_primary=True
    db.flush()
...
def _get_or_create_template(
    ...
    *,
    ...
    """
    Upsert GLEntryTemplate header. If is_primary=True, demote other primaries
    BEFORE setting this one to avoid unique violations.
    """
    tpl = db.scalar(
    ...
    if tpl:
        # If we are making this template primary, demote others first.
        if is_primary:
            _ensure_single_primary(
                db, company_id=company_id, doctype_id=source_doctype_id, keep_template_id=tpl.id
            )
        ...
        tpl.is_primary = bool(is_primary)
    ...
    # New template path
    if is_primary:
        # Demote ALL existing primaries for this doctype before inserting a new primary
        _ensure_single_primary(
            db, company_id=company_id, doctype_id=source_doctype_id, keep_template_id=None
        )
...
def _reset_items_for_template(
    ...
    for row in items:
        ...
        if not requires_dynamic:
            if not acct_code:
                logger.error(
                    "Template %s item missing static account_code and not dynamic; skipping.",
                    template_id,
                )
                continue
            ...
            if not acct:
                logger.error(
                    "Template %s item refers to unknown account_code %r; skipping.",
                    template_id,
                    acct_code,
                )
                continue
